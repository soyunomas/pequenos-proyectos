...
public class ScheduleEngineTest {
  ...
  @Test public void groupBelongsToSpecificAssignment(){Data d=new Data();d.subjects.add(new Subject("ROB","Robótica",0,TYPE_LECTIVA,"Sala 2.3"));d.setAssignment(0,MORNING,1,"ROB");d.setAssignment(1,MORNING,1,"ROB");d.setGroup(0,MORNING,1,"PE101 / TU101");d.setGroup(1,MORNING,1,"PA101");assertEquals("PE101 / TU101",d.getGroup(0,MORNING,1));assertEquals("PA101",d.getGroup(1,MORNING,1));Data copy=d.copy();assertEquals("PE101 / TU101",copy.getGroup(0,MORNING,1));d.setAssignment(0,MORNING,1,"BDD");assertEquals("",d.getGroup(0,MORNING,1));}
