...
public final class MainActivity extends Activity {
  ...
  private void render(){
    ...
    ScrollView sv=new ScrollView(this);HorizontalScrollView hs=new HorizontalScrollView(this);hs.setHorizontalScrollBarEnabled(false);hs.setPadding(dp(12),0,dp(12),dp(18));hs.addView(table(d,nn));sv.addView(hs);root.addView(sv,new LinearLayout.LayoutParams(-1,0,1));
  }
  ...
  private void shift(GridLayout g,Data d,ShiftConfig s,NowNext nn){if(!s.enabled)return;TextView head=t(s.label.toUpperCase(Locale.ROOT),13,true);head.setGravity(Gravity.CENTER);head.setTextColor(th.primarySoftText);head.setBackground(box(th.primarySoft,th.primarySoft,0,10));GridLayout.LayoutParams hp=new GridLayout.LayoutParams();hp.columnSpec=GridLayout.spec(0,6);hp.width=dp(378);hp.height=dp(38);hp.setMargins(dp(2),dp(8),dp(2),dp(2));hp.setGravity(Gravity.FILL);g.addView(head,hp);for(Slot slot:ScheduleEngine.generateSlots(d,s)){boolean rowCur=same(nn.current,slot),rowNext=same(nn.next,slot);add(g,timeCell(time(slot),rowCur||rowNext,rowCur),88,54,1);if(slot.isBreak)add(g,breakCell(rowCur,rowNext),290,54,5);else for(int day=0;day<5;day++){boolean cur=ScheduleEngine.matches(nn.current,day,slot),next=ScheduleEngine.matches(nn.next,day,slot);add(g,subjectCell(d.getAssignment(day,s.id,slot.sessionIndex),cur,next),58,54,1);}}}
  private TextView headerCell(String v){TextView x=t(v,12,true);x.setGravity(Gravity.CENTER);x.setPadding(dp(3),dp(3),dp(3),dp(3));x.setBackground(box(th.surfaceAlt,th.border,1,8));return x;}
  private TextView timeCell(String v,boolean highlighted,boolean current){TextView x=t(v,11,false);x.setGravity(Gravity.CENTER);x.setLineSpacing(0f,0.94f);x.setPadding(dp(3),0,dp(3),0);x.setTextColor(th.muted);int fill=current?th.primarySoft:highlighted?th.surfaceAlt:th.surface;x.setBackground(box(fill,highlighted?th.primary:th.border,highlighted?2:1,10));return x;}
  private TextView subjectCell(String raw,boolean current,boolean next){String code=(raw==null||raw.isEmpty())?"—":raw;String label=code;if(current)label+="\nAHORA";else if(next)label+="\nSIG.";TextView x=t(label,current||next?11:12,true);x.setGravity(Gravity.CENTER);x.setPadding(dp(2),0,dp(2),0);boolean empty="—".equals(code);int fill=empty?th.surfaceAlt:th.subjectColor(code);x.setTextColor(empty?th.muted:th.subjectTextColor(code));if(current||next){fill=blend(fill,th.primarySoft);x.setTextColor(th.dark?Color.WHITE:th.ink);}x.setBackground(box(fill,current||next?th.primary:(empty?th.border:fill),current||next?2:1,10));return x;}
  private TextView breakCell(boolean current,boolean next){String label="RECREO"+(current?"\nAHORA":next?"\nSIG.":"");TextView x=t(label,current||next?11:12,true);x.setGravity(Gravity.CENTER);x.setTextColor(th.dark?Color.rgb(255,231,173):Color.rgb(120,68,8));int fill=current||next?blend(th.breakBg,th.primarySoft):th.breakBg;x.setBackground(box(fill,current||next?th.primary:th.breakBorder,current||next?2:1,10));return x;}
  ...
  private Button button(String s){Button b=new Button(this);b.setText(s);b.setAllCaps(false);b.setTextColor(th.primaryText);b.setTypeface(b.getTypeface(),Typeface.BOLD);b.setMinHeight(0);b.setMinimumHeight(0);b.setPadding(dp(16),0,dp(16),0);b.setBackground(box(th.primary,th.primary,0,14));return b;}
