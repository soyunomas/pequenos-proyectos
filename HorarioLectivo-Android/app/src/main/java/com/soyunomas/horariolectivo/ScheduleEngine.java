...
public final class ScheduleEngine {
    ...
    public static List<Slot> generateAllSlots(Data data){List<Slot> all=new ArrayList<>();all.addAll(generateSlots(data,data.morning));all.addAll(generateSlots(data,data.afternoon));all.sort(Comparator.comparing(s->s.start));return all;}
    ...
    public static List<String> validate(Data data){List<String>e=new ArrayList<>();if(data==null){e.add("No hay datos de horario.");return e;}if(data.sessionMinutes<10||data.sessionMinutes>240)e.add("La duración de cada sesión debe estar entre 10 y 240 minutos.");validateShift(data.morning,e);validateShift(data.afternoon,e);if(!data.morning.enabled&&!data.afternoon.enabled)e.add("Activa al menos un turno.");if(data.morning.enabled&&data.afternoon.enabled&&data.afternoon.start.isBefore(data.morning.end))e.add("El turno de tarde no puede empezar antes de que termine el de mañana.");return e;}
    private static void validateShift(ShiftConfig s,List<String>e){if(!s.enabled)return;if(s.start==null||s.end==null||!s.start.isBefore(s.end))e.add("En "+s.label.toLowerCase()+", la hora de inicio debe ser anterior a la de fin.");if(s.breakAfterSession<0)e.add("La posición del recreo no puede ser negativa.");if(s.breakMinutes<0||s.breakMinutes>180)e.add("La duración del recreo debe estar entre 0 y 180 minutos.");}
    ...
    private static SlotRef toRef(Data d,int day,Slot s){return new SlotRef(day,s,s.isBreak?"RECREO":d.getAssignment(day,s.shiftId,s.sessionIndex));}
