...
public final class SettingsActivity extends Activity {
  ...
  private void editShift(GridLayout g,ShiftConfig s,int timeW,int dayW){
    ...
    for(Slot q:ScheduleEngine.generateSlots(d,s)){
      ...
      if(q.isBreak){
        for(int day=0;day<5;day++){int dd=day;String c=d.getAssignment(day,s.id,0),room=d.getRoom(day,s.id,0);TextView x=c.isEmpty()?breakCell():subjectCell(c,room);x.setContentDescription(c.isEmpty()?"Recreo. Toca para asignar una actividad":c+(room.isEmpty()?"":" en "+room)+" durante el recreo. Mantén pulsado para editar aula.");x.setOnClickListener(v->{set(dd,q.shiftId,0);renderEditor();});if(!c.isEmpty())x.setOnLongClickListener(v->{editRoom(dd,q.shiftId,0,c);return true;});add(g,x,dayW,54,1);}
      }else if(between){
