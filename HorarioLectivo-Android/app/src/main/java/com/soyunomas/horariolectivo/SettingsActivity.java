...
public final class SettingsActivity extends Activity {
  ...
  private void copyDay(int src,int dst){if(dst<0||src==dst)return;for(ShiftConfig s:new ShiftConfig[]{d.morning,d.between,d.afternoon,d.betweenNight,d.night})for(Slot q:ScheduleEngine.generateSlots(d,s)){d.setAssignment(dst,s.id,q.sessionIndex,d.getAssignment(src,s.id,q.sessionIndex));d.setRoom(dst,s.id,q.sessionIndex,d.getRoom(src,s.id,q.sessionIndex));}}
