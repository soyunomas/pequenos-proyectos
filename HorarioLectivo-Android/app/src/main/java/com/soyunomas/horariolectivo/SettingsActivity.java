...
public final class SettingsActivity extends Activity {
  ...
  private void render(){
    ...
    section("Apariencia","Ajusta el tema y decide si el widget muestra el aula/lugar.");LinearLayout dc=card();Switch dark=new Switch(this);dark.setText("Modo oscuro");dark.setTextColor(th.ink);dark.setChecked(repo.isDarkMode());dark.setOnCheckedChangeListener((b,on)->{repo.setDarkMode(on);render();});dc.addView(dark);divider(dc);Switch roomsWidget=new Switch(this);roomsWidget.setText("Mostrar aula/lugar en el widget");roomsWidget.setTextColor(th.ink);roomsWidget.setChecked(d.showRoomsInWidget);roomsWidget.setOnCheckedChangeListener((b,on)->d.showRoomsInWidget=on);dc.addView(roomsWidget);addCard(dc);
    ...
    section("2 · Asignaturas","L = lectiva · C = complementaria. Cada asignatura puede tener un aula general, además de siglas, nombre, tipo y color.");subjects();
    section("3 · Semana","Selecciona una asignatura y toca casillas. El grupo se muestra justo debajo de las siglas. Mantén pulsada una casilla asignada para editar el grupo y, si hace falta, el aula excepcional.");editor=new LinearLayout(this);editor.setOrientation(LinearLayout.VERTICAL);root.addView(editor);renderEditor();
  }
  ...
  private void shift(ShiftConfig s){
    ...
    if(s.enabled){
      if(d.hasCustomSlots(s.id)){
        divider(c);List<TimeSlotConfig> exact=d.customSlots(s.id);TextView exactInfo=text("Franjas exactas importadas: "+exact.size()+" · "+fmt(exact.get(0).start)+"–"+fmt(exact.get(exact.size()-1).end)+". Se respetan huecos y duraciones del horario original. Para cambiar estas horas, importa un JSON actualizado.",12,false);exactInfo.setTextColor(th.muted);c.addView(exactInfo);
      }else{value(c,"Comienzo",fmt(s.start),v->time(s.start,x->{s.start=x;render();}));divider(c);value(c,"Fin",fmt(s.end),v->time(s.end,x->{s.end=x;render();}));divider(c);value(c,"Recreo después de",s.breakAfterSession==0?"Sin recreo":"Sesión "+s.breakAfterSession,v->number("Posición del recreo",0,12,s.breakAfterSession,n->{s.breakAfterSession=n;render();}));if(s.breakAfterSession>0){divider(c);value(c,"Duración recreo",s.breakMinutes+" min",v->number("Duración recreo",5,90,Math.max(5,s.breakMinutes),n->{s.breakMinutes=n;render();}));}}
    }
    addCard(c);
  }
  ...
  private void betweenConfig(ShiftConfig gap,String title){
    ...
    if(gap.enabled){if(d.hasCustomSlots(gap.id)){divider(c);List<TimeSlotConfig> exact=d.customSlots(gap.id);TextView exactInfo=text("Franjas exactas importadas: "+exact.size()+" · "+fmt(exact.get(0).start)+"–"+fmt(exact.get(exact.size()-1).end)+". Para cambiar estas horas, importa un JSON actualizado.",12,false);exactInfo.setTextColor(th.muted);c.addView(exactInfo);}else{divider(c);value(c,"Comienzo",fmt(gap.start),v->time(gap.start,x->{gap.start=x;render();}));divider(c);value(c,"Fin",fmt(gap.end),v->time(gap.end,x->{gap.end=x;render();}));}}
    addCard(c);
  }
  ...
  private void subjects(){
    ...
    for(Subject s:new ArrayList<>(d.subjects)){
      ...
      LinearLayout meta=new LinearLayout(this);meta.setOrientation(LinearLayout.VERTICAL);TextView name=text(s.name,15,true);meta.addView(name);TextView type=text((s.isComplementaria()?"C":"L")+(s.defaultRoom.isEmpty()?"":" · "+s.defaultRoom),12,true);type.setContentDescription((s.isComplementaria()?"Complementaria":"Lectiva")+(s.defaultRoom.isEmpty()?"":". Aula general "+s.defaultRoom));type.setTextColor(s.isComplementaria()?th.breakBorder:th.muted);meta.addView(type);r.addView(meta,new LinearLayout.LayoutParams(0,-2,1));
  ...
  private void editSubject(Subject subject){
    ...
    TextView roomLabel=text("Aula general",12,true);roomLabel.setTextColor(th.muted);roomLabel.setPadding(0,dp(14),0,0);form.addView(roomLabel);EditText defaultRoom=input("Ej.: Aula PB.01");defaultRoom.setFilters(new InputFilter[]{new InputFilter.LengthFilter(80)});if(!creating)defaultRoom.setText(subject.defaultRoom);form.addView(defaultRoom,new LinearLayout.LayoutParams(-1,dp(52)));TextView roomHint=text("Se aplicará por defecto a todas las horas de esta asignatura. Las excepciones se editan después manteniendo pulsada una casilla.",12,false);roomHint.setTextColor(th.muted);roomHint.setPadding(0,dp(5),0,0);form.addView(roomHint);
    ...
    dialog.setOnShowListener(x->dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v->{String c=SubjectCode.normalize(code.getText().toString()),n=name.getText().toString().trim();if(!SubjectCode.isValid(c)){code.setError("Usa entre 1 y 3 caracteres válidos");return;}if(n.isEmpty()){name.setError("Escribe un nombre");return;}for(Subject other:d.subjects)if(other!=subject&&other.code.equalsIgnoreCase(c)){code.setError("Estas siglas ya existen");return;}String type=comp.isChecked()?TYPE_COMPLEMENTARIA:TYPE_LECTIVA;String generalRoom=defaultRoom.getText().toString().trim();if(creating){Subject s=new Subject(c,n,-1,type,generalRoom);d.subjects.add(s);selected=c;}else{d.renameSubjectCode(oldCode,c);subject.code=c;subject.name=n;subject.type=type;subject.defaultRoom=generalRoom;if(!generalRoom.isEmpty())d.rooms.entrySet().removeIf(e->c.equalsIgnoreCase(d.assignments.get(e.getKey()))&&generalRoom.equalsIgnoreCase(e.getValue()));if(oldCode.equalsIgnoreCase(selected))selected=c;}dialog.dismiss();render();}));dialog.show();
  }
  ...
  private void editShift(GridLayout g,ShiftConfig s,int timeW,int dayW){
    ...
    for(Slot q:ScheduleEngine.generateSlots(d,s)){
      TextView time=cell(String.format(Locale.US,"%02d:%02d\n%02d:%02d",q.start.getHour(),q.start.getMinute(),q.end.getHour(),q.end.getMinute()),between);if(between){time.setTextColor(th.breakBorder);time.setBackground(box(th.breakBg,th.breakBorder,1,8));}add(g,time,timeW,66,1);
      if(q.isBreak){
        for(int day=0;day<5;day++){int dd=day;String c=d.getAssignment(day,s.id,q.sessionIndex),group=d.getGroup(day,s.id,q.sessionIndex),room=d.getRoom(day,s.id,q.sessionIndex);TextView x=c.isEmpty()?breakCell():subjectCell(c,group,room);x.setContentDescription(c.isEmpty()?"Recreo. Toca para asignar una actividad":c+(room.isEmpty()?"":" en "+room)+" durante el recreo. Mantén pulsado para editar grupo y aula.");x.setOnClickListener(v->{set(dd,q.shiftId,q.sessionIndex);renderEditor();});if(!c.isEmpty())x.setOnLongClickListener(v->{editSlotDetails(dd,q.shiftId,q.sessionIndex,c);return true;});add(g,x,dayW,66,1);}
      }else if(between){
        for(int day=0;day<5;day++){int dd=day;String c=d.getAssignment(day,s.id,q.sessionIndex),group=d.getGroup(day,s.id,q.sessionIndex),room=d.getRoom(day,s.id,q.sessionIndex);TextView x=c.isEmpty()?betweenCell():subjectCell(c,group,room);x.setContentDescription(c.isEmpty()?"Entre turnos. Toca para asignar una actividad":c+(room.isEmpty()?"":" en "+room)+" entre turnos. Mantén pulsado para editar grupo y aula.");x.setOnClickListener(v->{set(dd,q.shiftId,q.sessionIndex);renderEditor();});if(!c.isEmpty())x.setOnLongClickListener(v->{editSlotDetails(dd,q.shiftId,q.sessionIndex,c);return true;});add(g,x,dayW,66,1);}
      }else for(int day=0;day<5;day++){int dd=day;String c=d.getAssignment(day,s.id,q.sessionIndex),group=d.getGroup(day,s.id,q.sessionIndex),room=d.getRoom(day,s.id,q.sessionIndex);TextView x=c.isEmpty()?subjectCell("·","",""):subjectCell(c,group,room);x.setOnClickListener(v->assign(dd,q));if(!c.isEmpty())x.setOnLongClickListener(v->{editSlotDetails(dd,q.shiftId,q.sessionIndex,c);return true;});add(g,x,dayW,66,1);}
    }
  }

  private TextView subjectCell(String code,String group,String room){boolean empty="·".equals(code);String g=group==null?"":group.trim(),r=room==null?"":room.trim();String label=empty?code:code+(g.isEmpty()?"":"\n"+g)+(r.isEmpty()?"":"\n"+r);TextView x=text(label,empty?12:(g.isEmpty()&&r.isEmpty()?12:9),true);x.setGravity(Gravity.CENTER);x.setMaxLines(3);x.setEllipsize(android.text.TextUtils.TruncateAt.END);x.setTextColor(empty?th.muted:th.subjectTextColor(code));x.setBackground(box(empty?th.surfaceAlt:th.subjectColor(code),empty?th.border:th.subjectColor(code),1,10));return x;}
  ...
  private void editSlotDetails(int day,String shift,int session,String code){
    Subject subject=d.subject(code);String general=subject==null?"":subject.defaultRoom;String override=d.getRoomOverride(day,shift,session);
    LinearLayout form=new LinearLayout(this);form.setOrientation(LinearLayout.VERTICAL);form.setPadding(dp(20),dp(8),dp(20),0);
    TextView groupLabel=text("Grupo",12,true);groupLabel.setTextColor(th.muted);form.addView(groupLabel);
    EditText group=input("Ej.: PA101 o PE101 / TU101");group.setFilters(new InputFilter[]{new InputFilter.LengthFilter(80)});group.setText(d.getGroup(day,shift,session));form.addView(group,new LinearLayout.LayoutParams(-1,dp(52)));
    TextView groupHint=text("Opcional. Se muestra justo debajo de las siglas en horario y widget.",12,false);groupHint.setTextColor(th.muted);groupHint.setPadding(0,dp(4),0,dp(12));form.addView(groupHint);
    TextView roomLabel=text("Aula excepcional",12,true);roomLabel.setTextColor(th.muted);form.addView(roomLabel);
    EditText room=input("Ej.: Sala 2.4");room.setFilters(new InputFilter[]{new InputFilter.LengthFilter(80)});room.setText(override);form.addView(room,new LinearLayout.LayoutParams(-1,dp(52)));
    TextView hint=text(general.isEmpty()?"No hay aula general para "+code+". Si quieres, indica aquí el aula de esta hora.":"Aula general: "+general+"\nDéjalo vacío para usar el aula general.",12,false);hint.setTextColor(th.muted);hint.setPadding(0,dp(5),0,0);form.addView(hint);
    new AlertDialog.Builder(this).setTitle("Detalles de esta hora").setView(form).setNeutralButton("Limpiar detalles",(a,b)->{d.setGroup(day,shift,session,"");d.setRoom(day,shift,session,"");renderEditor();}).setNegativeButton("Cancelar",null).setPositiveButton("Guardar",(a,b)->{d.setGroup(day,shift,session,group.getText().toString());d.setRoom(day,shift,session,room.getText().toString());renderEditor();}).show();
  }
  ...
  private void copyDay(int src,int dst){if(dst<0||src==dst)return;for(ShiftConfig s:new ShiftConfig[]{d.morning,d.between,d.afternoon,d.betweenNight,d.night})for(Slot q:ScheduleEngine.generateSlots(d,s)){d.setAssignment(dst,s.id,q.sessionIndex,d.getAssignment(src,s.id,q.sessionIndex));d.setRoom(dst,s.id,q.sessionIndex,d.getRoomOverride(src,s.id,q.sessionIndex));d.setGroup(dst,s.id,q.sessionIndex,d.getGroup(src,s.id,q.sessionIndex));}}
