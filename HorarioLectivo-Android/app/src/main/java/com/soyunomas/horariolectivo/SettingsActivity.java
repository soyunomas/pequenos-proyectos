...
public final class SettingsActivity extends Activity {
  ...
  private void render(){
    ...
    section("3 · Semana","Selecciona una asignatura y toca casillas. Mantén pulsada una casilla asignada para indicar su aula/lugar. Recreos e intervalos entre turnos también admiten asignatura y aula.");editor=new LinearLayout(this);editor.setOrientation(LinearLayout.VERTICAL);root.addView(editor);renderEditor();
  }
  ...
  private void shift(ShiftConfig s){
    ...
    if(s.enabled){
      if(d.hasCustomSlots(s.id)){
        divider(c);List<TimeSlotConfig> exact=d.customSlots(s.id);TextView exactInfo=text("Franjas exactas importadas: "+exact.size()+" · "+fmt(exact.get(0).start)+"–"+fmt(exact.get(exact.size()-1).end)+". Se respetan huecos y duraciones del horario original.",12,false);exactInfo.setTextColor(th.muted);c.addView(exactInfo);Button automatic=tonalButton("Volver a sesiones automáticas");automatic.setOnClickListener(v->{d.setCustomSlots(s.id,null);render();});LinearLayout.LayoutParams bp=new LinearLayout.LayoutParams(-1,dp(46));bp.setMargins(0,dp(10),0,0);c.addView(automatic,bp);
  ...
  private void betweenConfig(ShiftConfig gap,String title){
    ...
    if(gap.enabled){if(d.hasCustomSlots(gap.id)){divider(c);List<TimeSlotConfig> exact=d.customSlots(gap.id);TextView exactInfo=text("Franjas exactas importadas: "+exact.size()+" · "+fmt(exact.get(0).start)+"–"+fmt(exact.get(exact.size()-1).end)+".",12,false);exactInfo.setTextColor(th.muted);c.addView(exactInfo);Button automatic=tonalButton("Volver a intervalo automático");automatic.setOnClickListener(v->{d.setCustomSlots(gap.id,null);render();});LinearLayout.LayoutParams bp=new LinearLayout.LayoutParams(-1,dp(46));bp.setMargins(0,dp(10),0,0);c.addView(automatic,bp);}else{divider(c);value(c,"Comienzo",fmt(gap.start),v->time(gap.start,x->{gap.start=x;render();}));divider(c);value(c,"Fin",fmt(gap.end),v->time(gap.end,x->{gap.end=x;render();}));}}
    addCard(c);
  }
  ...
  private void editRoom(int day,String shift,int session,String code){
    EditText room=input("Ej.: Aula PB.01, Sala 2.4");room.setFilters(new InputFilter[]{new InputFilter.LengthFilter(80)});room.setText(d.getRoom(day,shift,session));room.setSelectAllOnFocus(false);
    LinearLayout form=new LinearLayout(this);form.setOrientation(LinearLayout.VERTICAL);form.setPadding(dp(20),dp(8),dp(20),0);TextView hint=text("Aula/lugar de "+code+". Puede ser distinto en cada día y hora.",12,false);hint.setTextColor(th.muted);hint.setPadding(0,0,0,dp(8));form.addView(hint);form.addView(room,new LinearLayout.LayoutParams(-1,dp(52)));
    new AlertDialog.Builder(this).setTitle("Aula o lugar").setView(form).setNeutralButton("Quitar aula",(a,b)->{d.setRoom(day,shift,session,"");renderEditor();}).setNegativeButton("Cancelar",null).setPositiveButton("Guardar",(a,b)->{d.setRoom(day,shift,session,room.getText().toString());renderEditor();}).show();
  }
