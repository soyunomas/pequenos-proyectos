...
public final class SettingsActivity extends Activity {
  ...
  private void render(){
    ...
    section("3 · Semana","Selecciona una asignatura y toca casillas. El aula general se hereda automáticamente; mantén pulsada una casilla asignada solo cuando esa hora tenga un aula excepcional.");editor=new LinearLayout(this);editor.setOrientation(LinearLayout.VERTICAL);root.addView(editor);renderEditor();
  }
  ...
  private void editShift(GridLayout g,ShiftConfig s,int timeW,int dayW){
    ...
    for(Slot q:ScheduleEngine.generateSlots(d,s)){
      ...
      if(q.isBreak){
        for(int day=0;day<5;day++){int dd=day;String c=d.getAssignment(day,s.id,q.sessionIndex),room=d.getRoom(day,s.id,q.sessionIndex);TextView x=c.isEmpty()?breakCell():subjectCell(c,room);x.setContentDescription(c.isEmpty()?"Recreo. Toca para asignar una actividad":c+(room.isEmpty()?"":" en "+room)+" durante el recreo. Mantén pulsado para editar aula.");x.setOnClickListener(v->{set(dd,q.shiftId,q.sessionIndex);renderEditor();});if(!c.isEmpty())x.setOnLongClickListener(v->{editRoom(dd,q.shiftId,q.sessionIndex,c);return true;});add(g,x,dayW,54,1);}
      }else if(between){
        for(int day=0;day<5;day++){int dd=day;String c=d.getAssignment(day,s.id,q.sessionIndex),room=d.getRoom(day,s.id,q.sessionIndex);TextView x=c.isEmpty()?betweenCell():subjectCell(c,room);x.setContentDescription(c.isEmpty()?"Entre turnos. Toca para asignar una actividad":c+(room.isEmpty()?"":" en "+room)+" entre turnos. Mantén pulsado para editar aula.");x.setOnClickListener(v->{set(dd,q.shiftId,q.sessionIndex);renderEditor();});if(!c.isEmpty())x.setOnLongClickListener(v->{editRoom(dd,q.shiftId,q.sessionIndex,c);return true;});add(g,x,dayW,54,1);}
      }else for(int day=0;day<5;day++){int dd=day;String c=d.getAssignment(day,s.id,q.sessionIndex),room=d.getRoom(day,s.id,q.sessionIndex);TextView x=c.isEmpty()?subjectCell("·",""):subjectCell(c,room);x.setOnClickListener(v->assign(dd,q));if(!c.isEmpty())x.setOnLongClickListener(v->{editRoom(dd,q.shiftId,q.sessionIndex,c);return true;});add(g,x,dayW,54,1);}
    }
  }

  private TextView subjectCell(String code,String room){boolean empty="·".equals(code);String label=empty?code:code+(room==null||room.trim().isEmpty()?"":"\n"+room.trim());TextView x=text(label,empty?12:(room==null||room.trim().isEmpty()?12:9),true);x.setGravity(Gravity.CENTER);x.setMaxLines(2);x.setEllipsize(android.text.TextUtils.TruncateAt.END);x.setTextColor(empty?th.muted:th.subjectTextColor(code));x.setBackground(box(empty?th.surfaceAlt:th.subjectColor(code),empty?th.border:th.subjectColor(code),1,10));return x;}
  ...
  private void editRoom(int day,String shift,int session,String code){
    ...
    EditText room=input("Ej.: Sala 2.4");room.setFilters(new InputFilter[]{new InputFilter.LengthFilter(80)});room.setText(override);room.setSelectAllOnFocus(false);
    LinearLayout form=new LinearLayout(this);form.setOrientation(LinearLayout.VERTICAL);form.setPadding(dp(20),dp(8),dp(20),0);
    TextView hint=text(general.isEmpty()?"No hay aula general configurada para "+code+". Escribe aquí el aula de esta hora.":"Aula general: "+general+"\nSolo escribe aquí un aula si esta hora es una excepción.",12,false);hint.setTextColor(th.muted);hint.setPadding(0,0,0,dp(8));form.addView(hint);form.addView(room,new LinearLayout.LayoutParams(-1,dp(52)));
    String neutral=general.isEmpty()?"Quitar aula":"Usar aula general";
    new AlertDialog.Builder(this).setTitle("Aula excepcional").setView(form).setNeutralButton(neutral,(a,b)->{d.setRoom(day,shift,session,"");renderEditor();}).setNegativeButton("Cancelar",null).setPositiveButton("Guardar excepción",(a,b)->{d.setRoom(day,shift,session,room.getText().toString());renderEditor();}).show();
  }
  ...
  private void copyDay(int src,int dst){if(dst<0||src==dst)return;for(ShiftConfig s:new ShiftConfig[]{d.morning,d.between,d.afternoon,d.betweenNight,d.night})for(Slot q:ScheduleEngine.generateSlots(d,s)){d.setAssignment(dst,s.id,q.sessionIndex,d.getAssignment(src,s.id,q.sessionIndex));d.setRoom(dst,s.id,q.sessionIndex,d.getRoomOverride(src,s.id,q.sessionIndex));}}
